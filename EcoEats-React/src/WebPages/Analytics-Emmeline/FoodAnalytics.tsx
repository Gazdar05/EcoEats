...
export default function FoodAnalytics() {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      // Summary
      const summaryRes = await fetch(`${API_BASE_URL}/analytics/summary?${params}`);
      if (!summaryRes.ok) throw new Error('Failed to fetch summary');
      const summaryData = await summaryRes.json();
      setSummary(summaryData);

      // Categories
      const categoryRes = await fetch(`${API_BASE_URL}/analytics/categories?${params}`);
      if (!categoryRes.ok) throw new Error('Failed to fetch categories');
      const categoryDataRaw = await categoryRes.json();
      const catData = Object.entries(categoryDataRaw.categoriesBreakdown || {}).map(([name, data]: any) => ({
        name,
        total: data.total,
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `ecoEats-analytics-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
  ...
  if (error) return (
    <>
      ...
      <div className="error-container">
        <h2>Error</h2>
        <p>{error}</p>
        <button className="retry-btn" onClick={() => (window.location.href = '/analytics')}>
          Retry
        </button>
      </div>
    </>
  ...
  if (!summary?.hasData) return (
    <>
      ...
      <div className="no-data-container">
        ...
        <button className="no-data-btn" onClick={() => (window.location.href = '/inventory')}>
          Go to Inventory
        </button>
      </div>
    </>
      ...
      <div className="analytics-wrapper">
        <div className="analytics-container">
          ...
          <div className="filters-card">
            <h3><Calendar size={18}/> Filters</h3>
